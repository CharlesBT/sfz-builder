...
export class sfzBuilder {
  ...
  static async getPatchNamesFromWavFiles(inputPath: string, options: sfzOptions = {}) {
    ...
    for (const file of files) {
      ...
      const regex = new RegExp(options?.process?.wav_regexp as string, 'g')
      try {
        const patchName = regex.exec(filename)[1]
        // const [[, patchName]] = [...filename.matchAll(regex)]
  ...
  static async processPatchFromWav(patchFileName: string, options: sfzOptions = {}) {
    ...
    for (const wav of samples) {
      ...
      // const [[, , rootkey, velocity]] = [...filename.matchAll(wavRegExp)]
  ...
  static getKeyIntervalsFromFiles(files: string[], options: sfzOptions) {
    ...
    for (const file of files) {
      ...
      // const [[, , rootkey]] = [...filename.matchAll(regex)]
  ...
  static getVelocityMapFromFiles(files: string[], options: sfzOptions) {
    ...
    for (const file of files) {
      ...
      const regex = _.clone(wavRegExp)
      const regexResult = regex.exec(filename)
      // const [[, , regexResult]] = [...filename.matchAll(regex)]
