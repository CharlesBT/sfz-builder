...
let __assign =
  ...
  function (...args) {
    __assign =
      Object.assign ||
      function (t, ...args) {
        for (let s, i = 0, n = args.length; i < n; i++) {
          s = args[i]
...
export function parseSfzSection(sfzText) {
  // remove comments
...
function applyScopeHeaders(sfz) {
  ...
  return sfz
    .map(function (s) {
      if (s.section === 'global') global = s
      else if (s.section === 'group') group = s
      else {
        if (global) s.property = __assign(__assign({}, s.property), global.property)
        if (group) s.property = __assign(__assign({}, s.property), group.property)
        return s.property
      }
    })
    .filter(function (s) {
...
export function parseSfz(sfzText) {
  return applyScopeHeaders(parseSfzSection(sfzText))
}
