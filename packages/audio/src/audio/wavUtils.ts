...
export class wavUtils {
  ...
  static async writeJsonInfo(file: string) {
    ...
    if (wavloop !== undefined) {
      ...
      loop.start_time = _.round(wavloop.dwStart / samplerate, 10)
      loop.end_time = _.round(wavloop.dwEnd / samplerate, 10)
    }
    info.loop = loop
  ...
  static async updateJsonInfo(file: string, info: IWavInfo) {
    await fs.promises.writeFile(file + '.json', JSON.stringify(info))
  }
  ...
  static async jsonHasLoop(file: string) {
    const info = await wavUtils.getJsonInfo(file)
  ...
  static async writeLoopPoints(file: string, loopstart: number, loopend: number) {
    const ext = 'loop.wav'
  ...
  static async processAppliedToWav(file: string, info: IWavInfo, processname: string) {
    // write loop points
  ...
  static async processConvertToMaxSampleRate(file: string, info: IWavInfo) {
    const samplerate = parseInt(info.streams[0].sample_rate, 10)
  ...
  static async processConvertToMaxBitDepth(file: string, info: IWavInfo) {
  ...
  static async processVolume(file: string, volume: number) {
    const ext = 'vol.wav'
    const info = await wavUtils.getJsonInfo(file)
  ...
  static async processFadeOut(file: string) {
  ...
  static async processRemoveSilence(file: string) {
    // return if json file contains loop points
    if (await wavUtils.jsonHasLoop(file)) {
      return
    }
