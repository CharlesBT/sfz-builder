...
import { config, pkgRoot } from '@bms/common'
...
export class ffmpeg {
  static async execCmd(cmd) {
    ...
    const promise = exec(cmd, { timeout: ENCODER_TIMEOUT })
    ...
    // })
    // child.on('close', (code) => {
    //     return code === 0 ? true : false // success when code === 0
    // })
    // child.on('error', (err) => {
  ...
  static getFFmpegCmd(inputFile, outputExt, encodingOptions) {
  ...
  static async writeNbSamplesAndMaxVolume(file) {
    const relativePosixInputFilepath = path
      .relative(__pkgRoot, file)
      .split(path.sep)
      .join(path.posix.sep)
    ...
    const nb_samples = parseInt(nbsamplesRe.exec(txt)[1].trim(), 10)
    info.streams[0].nb_samples = nb_samples

    if (nb_samples > 0) {
      const maxvolRe = _.clone(maxvolRegex)
      const maxvolume = parseInt(maxvolRe.exec(txt)[1].trim(), 10)
      const meanvolRe = _.clone(meanvolRegex)
      const meanvolume = parseInt(meanvolRe.exec(txt)[1].trim(), 10)
