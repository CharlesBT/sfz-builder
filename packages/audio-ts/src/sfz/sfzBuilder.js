...
import { config, filer, uuid } from '@bms/common'
...
export class sfzBuilder {
  ...
  static async processAllPatches(inputPath, options = {}) {
    ...
    options.process.autorename_wavfile
      ? await sfzBuilder.autoRenameFolder(inputPath, options)
      : null

    let patchFileNames, patchProcessingFunction
    switch (options.process.patchname_from) {
      case 'sfz':
      ...
      case 'wav':
        ...
        break
      case 'folder':
        patchFileNames = await sfzBuilder.getPatchNamesFromFolderNames(inputPath, options) // detect patches names in inputPath
  ...
  static async getPatchNamesFromSfzFiles(inputPath, options = {}) {
    let patchFiles = []
    ...
    return patchFiles.sort((a, b) => a - b)
  }

  // extract patchnames from WAV files and returns an array of patches to process
  static async getPatchNamesFromWavFiles(inputPath, options = {}) {
    let files
    options.process.recursive
    ...
    for (const file of files) {
      ...
      const regex = new RegExp(options.process.wav_regexp, 'g')
      try {
        const patchName = regex.exec(filename)[1]
  ...
  static async processPatchFromSfz(patchFileName, options = {}) {
    ...
    if (options.process.update_sfzfile) {
      ...
      for (const elt of sfz) {
        switch (elt.section) {
          ...
          case 'region':
            {
              const wav = filer.sanitizeName(elt.property.sample)
              ...
              for (const [key, value] of Object.entries(elt.property)) {
                key !== 'sample' ? region.set(key, value) : null
              }
  ...
  static async processPatchFromWav(patchFileName, options = {}) {
    console.info(`Processing patch: ${patchFileName}`)

    const patchName = path.parse(patchFileName).name
    const inputPath = path.parse(patchFileName).dir
    const midiKeyMap = options.process.midikeymap
    const patch = new sfzPatch(options.patch)
    patch.name = patchName
    ...
    if (options.patch.type !== 'drumkit' && !keyIntervals.every((elt) => elt === keyIntervals[0])) {
    ...
    for (const wav of samples) {
      ...
      const wavRegExp = new RegExp(options.process.wav_regexp, 'g')
      const wavRegExpResult = wavRegExp.exec(filename)
      const rootkey = wavRegExpResult[2]
      const region = new sfzRegion(filename)
      region.set('pitch_keycenter', midiKeyMap.get(rootkey))
      // set lokey and hikey
      switch (keyIntervals[0]) {
        case 1:
          region.set('lokey', region.get('pitch_keycenter'))
          region.set('hikey', region.get('pitch_keycenter'))
          break
        case 3:
          region.set('lokey', region.get('pitch_keycenter') - 1)
          region.set('hikey', region.get('pitch_keycenter') + 1)
          break
        case 6:
          region.set('lokey', region.get('pitch_keycenter') - 2)
          region.set('hikey', region.get('pitch_keycenter') + 3)
          break
        case 12:
          region.set('lokey', region.get('pitch_keycenter') - 5)
          region.set('hikey', region.get('pitch_keycenter') + 6)
          break
        default:
          region.set('lokey', region.get('pitch_keycenter'))
          region.set('hikey', region.get('pitch_keycenter'))
      ...
      let layer = parseInt(wavRegExpResult[3], 10)
  ...
  static async getPatchNamesFromFolderNames(inputPath) {
  ...
  static async processPatchFromFolderName(patchFileName, options = {}) {
    console.info(`Processing patch: ${patchFileName}`)
    ...
    // process each files
    let startkeyInit, startkey
    options.patch.startkey
      ? (startkeyInit = midiKeyMap.get(options.patch.startkey))
      : (startkeyInit = 0)
  ...
  // move samples to default_path folder
  static async movePatchSamplesToDefaultPath(samples, patchFileName, patch) {
    const sampleFolderPath = path.join(patchFileName, patch.get('default_path'))
  ...
  static async getProcessPatchSamples(inputFiles, options = { process: {}, patch: {} }) {
    // create temp folder
    const inputFolder = path.parse(inputFiles[0]).dir
    const tmpFolder = path.resolve(path.join(config.folders.temp, uuid.v4()))
        ...
        case 'patch':
          {
            ...
            tasks = files.map(async (file) => {
              await wavUtils.processVolume(file, volume)
            })
            await Promise.all(tasks)
          }
          break
  ...
  static async getPatchSamples(patchFileName, options) {
    const patchName = path.parse(patchFileName).name
  ...
  static async getMaxVolume(files) {
  ...
  static getKeyIntervalsFromFiles(files, options) {
    const wavRegExp = new RegExp(options.process.wav_regexp, 'g')
    const midiKeyMap = options.process.midikeymap
    const keyMap = []
    for (const file of files) {
      const filename = path.parse(file).base
      const regex = _.clone(wavRegExp)
      const regexResult = regex.exec(filename)
      const rootkey = regexResult[2]
  ...
  static getVelocityMapFromFiles(files, options) {
    const wavRegExp = new RegExp(options.process.wav_regexp, 'g')
    const velocityTransformer = options.process.velocitytransformer
    const velocityMap = []
    for (const file of files) {
      ...
      const regex = _.clone(wavRegExp)
      const regexResult = regex.exec(filename)
      let layer = parseInt(regexResult[3], 10)
      if (typeof velocityTransformer === 'function') {
        layer = velocityTransformer(layer)
      }
  ...
  static getGroupsFromVelocityMap(velocityMap) {
    const groups = []
  ...
  static async removeEmptyFiles(files, options) {
    if (options.process.remove_empty_wavfile) {
  ...
  static async autoRenameFolder(inputPath, options) {
    if (options.process.autorename_wavfile) {
      // auto rename folders
  ...
  static async autoRenameWavFiles(inputPath, options) {
    if (options.process.autorename_wavfile) {
      // auto rename files
      const directoryContent = await fsExtra.promises.readdir(inputPath, {
        withFileTypes: true,
